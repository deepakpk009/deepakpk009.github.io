...
/**
 * Create a file list item element with updated structure for chunk progress
...
const progressContainer = createProgressBar('file-progress');

// Add chunk progress elements
const chunkStatus = document.createElement('div');
chunkStatus.className = 'chunk-status';
chunkStatus.style.display = 'none';

const chunkProgressContainer = createProgressBar('chunk-progress');
...
fileItem.appendChild(chunkStatus);
fileItem.appendChild(chunkProgressContainer);
...
* @param {string} className - Additional class name for the progress bar
* @returns {HTMLElement} - The progress container element
*/
function createProgressBar(className = '') {
...
try {
  ...
  // Disable remove buttons for all files
  ...
  // Re-enable the upload button and remove buttons
  DOM.uploadButton.disabled = false;
  enableUnuploadedRemoveButtons();
}
}
...
const removeButtons = DOM.fileList.querySelectorAll('button');
...
/**
* Enable remove buttons for files that haven't been uploaded
*/
function enableUnuploadedRemoveButtons() {
...
fileItems.forEach(item => {
  ...
  // Only enable remove buttons for files not completed or currently uploading
  if (status === 'Waiting' || status.includes('Failed')) {
...
const chunkStatus = fileItem.querySelector('.chunk-status');
const chunkProgressContainer = fileItem.querySelector('.progress-container:nth-of-type(2)');
const chunkProgressBar = chunkProgressContainer.querySelector('.progress-bar');
...
try {
  await uploadFileInChunks(file, progressBar, fileStatus, chunkProgressBar, chunkStatus);
...
* @param {HTMLElement} progressBar - File progress bar element
* @param {HTMLElement} fileStatus - File status element
* @param {HTMLElement} chunkProgressBar - Chunk progress bar element
* @param {HTMLElement} chunkStatus - Chunk status element
*/
async function uploadFileInChunks(file, progressBar, fileStatus, chunkProgressBar, chunkStatus) {
const CHUNK_SIZE = calculateOptimalChunkSize(file.size);
const totalChunks = Math.ceil(file.size / CHUNK_SIZE);

for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex++) {
  // Reset chunk progress bar for each new chunk
  chunkProgressBar.style.width = '0%';
  
  await uploadChunk(file, chunkIndex, totalChunks, CHUNK_SIZE, progressBar, fileStatus, chunkProgressBar, chunkStatus);
}
}

/**
* Upload a single chunk of a file with detailed progress
...
* @param {HTMLElement} chunkStatus - Chunk status element
*/
async function uploadChunk(file, chunkIndex, totalChunks, chunkSize, progressBar, fileStatus, chunkProgressBar, chunkStatus) {
...
while (!uploadSuccess && retries < CONFIG.MAX_RETRIES) {
  try {
    ...
    updateChunkStatus(chunkStatus, chunkIndex, totalChunks, retries);
    
    // Create form data
    const formData = createChunkFormData(file, chunk, chunkIndex, totalChunks);
    
    // Create upload handler with progress tracking
    const { controller, progressPromise } = createProgressTracker(chunkProgressBar, chunkStatus);
    
    // Start progress simulation
    progressPromise.catch(() => {}); // Catch any aborts
    ...
    await handleRetryBackoff(chunkStatus, retries);
  }
}
}

/**
* Create a progress tracker for chunk uploads
* @param {HTMLElement} progressBar - Progress bar to update
* @param {HTMLElement} statusElement - Status element to update
* @returns {Object} - Controller to abort and progress promise
*/
function createProgressTracker(progressBar, statusElement) {
const controller = new AbortController();

const progressPromise = new Promise((resolve, reject) => {
  // Track whether we've been aborted
  controller.signal.addEventListener('abort', () => {
    reject(new Error('Progress tracking aborted'));
  });
  ...
  const simulateProgress = () => {
    ...
    // Simulate upload progress - cap at 95% until we get confirmation
    if (progress < 95) {
      // Slower progress as we approach 95%
      const increment = progress < 50 ? 5 : (progress < 80 ? 2 : 0.5);
      progress += increment;
      
      // Update UI
      updateChunkProgressBar(progressBar, statusElement, progress);
...
return { controller, progressPromise };
}

/**
* Update chunk progress bar and text
* @param {HTMLElement} progressBar - Chunk progress bar to update
* @param {HTMLElement} statusElement - Status element containing chunk info
* @param {number} progress - Progress percentage (0-100)
*/
function updateChunkProgressBar(progressBar, statusElement, progress) {
const roundedProgress = Math.round(progress);
progressBar.style.width = `${roundedProgress}%`;

// Append progress % to the existing chunk status text
const currentText = statusElement.textContent;
const baseText = currentText.replace(/ - \d+%$/, '');
statusElement.textContent = `${baseText} - ${roundedProgress}%`;
}
...
*/
function updateChunkStatus(chunkStatus, chunkIndex, totalChunks, retries) {
chunkStatus.textContent = retries > 0 ? 
